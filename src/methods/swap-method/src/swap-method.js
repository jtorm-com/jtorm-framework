/*! (c) jTorm and other contributors | www.jtorm.com/license */
'use strict';
module.exports = {
    jTormSwapMethod: {
        alias: 's',
        params: ['s', 'a', 'ui', 'h'],
        validate: function (j, v) {
            return !!v.d.s;
        },
        handle: async function (j, v) {
            var w = v.d.s ? v.d.s : v.c.s, a = v.d.a, i;
// console.log('begin swap', v.t, v.h.html(), v.d);
            if (v.d.ui || v.d.h) {
                // h, t, m, c, v
                var jD2 = await j.handle(v.d.h ? v.d.h : '', v.d.ui ? [ { s: false, m: 'ui', p: { c: '"' + v.d.ui + '"', t: '0' }, c: [] } ]: [], {}, 1);
                v.d.s = jD2.select(v.d.s);
            }
// console.log('dezee swap', jD2.html(), v.t, v.h.html());
            await v.h.set(v.t.s, function (el) {
                if (typeof w === 'string')
                    w = v.h.d.createElement(w);

                w.innerHTML = el.innerHTML;
                if (a && el.hasAttributes()) {
                    a = el.attributes;
                    for (i = a.length - 1; i >= 0; i--)
                        if (!w.getAttribute(a[i].name))
                            w.setAttribute(a[i].name, a[i].value);
                }
                el.parentNode.replaceChild(w, el);
            }, v);

            v.io = {c: 1};
        }
    }
};