...
- **Persisted rendered-fragment age and live publication have two policy owners.**
  `@jtorm/ui-cache-model` owns the public versioned save-adapter envelope, restart-stable Unix-ms
  `settledAt`, whole-envelope validation, exact byte/timestamp pairing, bounded LRU persistence,
  exact rendered-refresh generations/capabilities, transactional live publication plus root-dirty
  rollback, and the migration boundary. `@jtorm/promise-cache-model` alone owns current
  finite/zero/Infinity TTL and stale-phase classification, opaque process-local insertion records
  and rollback checkpoints, strict non-sliding freshness, and process-clock identity. Reload
  requires own data-property `saveModel.uiCacheScoped === true` plus own recognized wire version;
  never invent timestamps, serialize clock/context/capability identity, or expose insertion records.
- **Request-triggered stale service has distinct acquisition and rendered paths.**
  `@jtorm/promise-cache-model` centrally owns the strict finite `staleWindow` state machine for data,
  HTML, TSS, and UI-manifest pack promises. Those four owners default to `0`; one stale request may
  start a retained-generation refresh, success publishes at fulfillment, failure never slides the
  hard deadline and may be retried by a later eligible acquisition, and hard callers join running
  work. Manifest reuse re-runs URL policy and the exact captured-key check; data/HTML/TSS retain
  their key-at-call admission model and require purge when host policy tightens.
  Rendered fragments opt in independently through `ui-cache-model.staleWindow` (default `0`) and
  one stable `refreshModel` injected into both UI-cache model and plugin before `init()`. That host
  alone owns canonical input snapshots, isolated roots, lifecycle execution, platform lifetime,
  cancellation, and observability through `authorize/current/render/session/owns`; current and
  ownership decisions must return literal `true`. The cache issues a frozen zero-key one-shot
  capability, permits one attempt per exact process generation, and starts no timer/job/retry.
  Stale callers receive exact retained HTML; hard callers never do and authorized independent roots
  join the private refresh promise, then reauthorize and reclassify before consuming it, while the
  refresh root bypasses itself. Before the target iteration binds, any coordinate mismatch or
  missing/malformed current scope fails loud before handler effects; afterward, unrelated nested
  component coordinates retain ordinary lookup. Cache publication occurs only in the normal
  completion hook after current scope/authority/session/generation checks and is
  transactional with the isolated root's dirty revision; the host observes the later after-view
  save, whose external ordering/durability remain adapter-owned. If an earlier after-view hook
  aborts after publication but before that save hook, host rejection closes the completed execution
  without rolling back live bytes; dirty state remains retryable and no save is claimed. A restored stale wire-v1 record is
  a new process generation and is accepted only while the same valid host remains configured.
  Missing/malformed policy or collaborator behaves as zero for rendered stale service. Purge,
  eviction, init, cache/order replacement, newer publication, scope drift, or authority rotation
  detaches late publication. Sensitive rollback sets both stale windows to zero, disposes roots,
  purges live/persisted fragments, and isolates or cold-clears old readers before downgrade.
- **HTTP validators are opt-in and acquisition-only.** Data, HTML, TSS, and UI-manifest owners expose exact boolean `validators`, default `false`; rendered fragments expose no validator policy. `request-model.conditional()` alone owns URL resolution, timeout, awaited allow policy, resolved-URL scoped-key recheck, one bounded opaque ETag/Last-Modified condition, and 200/304 response envelopes. `promise-cache-model` pairs at most one validator inside the exact private Map/key/promise/generation record and exposes a loader-local transaction; public Maps remain `Map<key, Promise>`. A 304 may publish a fresh generation only by reusing the exact current authorized content+validator pair; a 200 stages replacement metadata only after JSON/text/TSS/complete manifest validation, and missing metadata clears the predecessor. Purge/reset/eviction/Map replacement/rejection/newer work detach both metadata and reuse/publication authority. Keep values bounded/opaque, preserve exact legacy loaders when disabled/unscoped/unsupported, and do not add persistence, cache-control semantics, timers, retries, new stale fallback, rendered-fragment behavior, or runtime imports.
...
- **The frozen v1 `tss-parser` oracle arithmetic is historical evidence** — the brace
  ...
  bounded tokenizer/recursive-descent implementation, locked against all 285 `src/**/*.tss` files.
