...
- **Persisted rendered-fragment age has two policy owners.** `@jtorm/ui-cache-model` owns the public versioned save-adapter envelope, restart-stable Unix-ms `settledAt`, whole-envelope validation, exact byte/timestamp pairing, bounded LRU persistence, and migration/rollback boundary; its exported live `cache` remains the nested HTML-only shape. `@jtorm/promise-cache-model` alone restores opaque process-local insertion records and owns current finite/zero/Infinity TTL, strict non-sliding freshness, and process-clock identity. Reload requires own data-property `saveModel.uiCacheScoped === true` plus own recognized wire version; legacy/unversioned stores are cold-cleared or externally migrated only with trustworthy original timestamps. Hosts finish `init()` before render roots, use a nondecreasing restart-stable Unix clock, isolate mixed reader versions, and clear/restore/disable persistence before downgrade. Do not invent reload/save/hit timestamps, parallel unpaired persistence metadata, serialized clock/context identities, or rendered-fragment stale fallback/refresh.
- **Request-triggered stale service is acquisition-only.** `@jtorm/promise-cache-model` centrally owns the strict finite `staleWindow` state machine for data, HTML, TSS, and UI-manifest pack promises; those four owners expose default `0`, while `@jtorm/ui-cache-model` exposes no stale window. At `ttl <= age < ttl + staleWindow`, the caller receives the old public promise and at most one retained-generation refresh starts; success publishes the exact refresh promise at fulfillment, failure never slides the old hard deadline, and hard callers join running refresh work. A missing or invalid window behaves as zero without discarding ordinary TTL freshness, `ttl=0` stays pending-only, and `ttl=Infinity` stays non-expiring. Unscoped keys bypass cache/policy/clock entirely. Manifest guarded reuse must re-run URL policy and rederive the exact captured key after the await before any stale service or refresh; data/HTML/TSS retain their pre-existing key-at-call admission model and require purge when host policy tightens. Acquisition purge/reset/eviction prevents future publication but never aborts detached work or revokes already returned/derived/root-local/rendered/persisted state; sensitive rollback also disposes roots and purges/saves UI persistence. Do not add timers, refresh-ahead, backoff, durable/serverless completion claims, rendered-fragment SWR, or runtime imports.
