...
test('returns text and does not cache a rejection (#16)', async () => {
  hm.c = new Map();
  hm.requestModel = { cacheKey: u => String(u), get: () => ({ text: () => Promise.reject(new Error('net')) }) };
  await assert.rejects(() => hm.get('/x'));
  hm.requestModel = { cacheKey: u => String(u), get: () => ({ text: () => Promise.resolve('<b>hi</b>') }) };
...
test('concurrent callers share one in-flight fetch', async () => {
  ...
  hm.requestModel = { cacheKey: u => String(u), get: () => ({ text: () => {
    fetches++;
...
test('bounds the cache to `max` entries — evicts oldest, no unbounded growth', async () => {
  ...
  hm.requestModel = { cacheKey: u => String(u), get: (u) => ({ text: () => Promise.resolve(u) }) };
...
test('caches and returns an integer-like key (LRU order must not depend on key type)', async () => {
  hm.c = new Map(); hm.max = 2;
  hm.requestModel = { cacheKey: u => String(u), get: (u) => ({ text: () => Promise.resolve('v' + u) }) };
  await hm.get('/a');
...
test('a repeated array-of-URLs key reuses the cache across distinct array instances', async () => {
  hm.c = new Map(); hm.max = 512;
  let fetches = 0;
  hm.requestModel = { cacheKey: u => String(u), get: () => ({ text: () => { fetches++; return Promise.resolve('X'); } }) };
